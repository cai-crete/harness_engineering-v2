...
```
project_[node_name]/
├── index.md                               ← 노드 폴더 체계 인덱스 (역할·Agent 소유 정의)
├── _context/                              ← 노드 전용 하네스 (Protocol + 디자인 컨텍스트)
│   ├── protocol/                          ← Protocol 버전 아카이브 (삭제 금지)
...
│   ├── business-context.md                ← 비즈니스 배경·서비스 가치 (AGENT A 참조)
...
└── lib/                                   ← 공유 로직 레이어 (AGENT A 관리, 목적에 따라 확장 가능)
    └── prompt.ts                          ← buildSystemPrompt() 구현 (필수)
```
...
```
_context/protocol/protocol-[node_name]-v[N].txt    ← 버전 명시 필수
```
